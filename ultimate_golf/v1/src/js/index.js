function createChart() {
  var chart = new CanvasJS.Chart("chartContainer", {
	...
	subtitles: [{
		text: "Test data until we have some fake stats to show"
	}],
	axisX: {
		lineColor: "black",
...
$(document).ready(function() {
  $("#main_video").hover(function(e) {
    console.log(e)
    if(e.type === "mouseenter") {
      $("#play_button").addClass("play_button_hover");
    } else {
      $("#play_button").removeClass("play_button_hover");
    }
  })
...
$(window).scroll(function (event) {
    ...
    if (scroll == 0)
        ...
        $( "#navbar" ).css( "background-color", "rgba(20, 20, 40, 0.8)");
});
...
$(function() {
    $("#animation").mousemove(function(e) {
        ...
        var w = e.pageX - parentOffset.left;
        var frameNumber = w / ($(this).width() + 30);
        var currentDisplacement = Math.abs(frameNumber * vid.duration - last);
        $("#anim_text").html(currentDisplacement);
        if (currentDisplacement > 1/69) {
            last = frameNumber * vid.duration;
            console.log(last);
            vid.currentTime  = last;
        }
    })
});
